...
<div class="container">
    <div class="justify-content-center">
        <div class="text-center my-3">
            <h2 class="title display-3">Ficha de Herramienta</h2> 
        </div>
        <hr class="border-red">
        @if(session('message'))
                    ...
                    <h5 class="mx-3"> 
                        <div class="row my-2">   
                            <div class="form-group col-md-4">
                                <label class="title">{{ __('Herramienta') }}</label><br>
                                {{$herramienta->nombre}}      
                            </div>
                            <div class="form-group col-md-4">
                                <label class="title">{{ __('Cantidad') }}</label><br>
                                {{$herramienta->cantidad}}
                            </div>
                            <div class="form-group col-md-4">
                                <label class="title">{{ __('Ocupadas') }}</label><br>
                                @if(count($herramienta->asignadas)>0)
                                    @php($cant=0)
                                    @foreach($herramienta->asignadas as $as)
                                        @if($as->fechaDevolucion==NULL)
        ...
        <div class="card">
          ...
          <div class="card-body bg-camel">
            ...
            <form class="form" method="POST" action="{{route('herramienta.prestar')}}">
              @csrf
              <div class="row">
                <div class="form-group col-md-6">
                  <label for="nombre" class="title">Personal</label>
                  <input type="hidden" name="id_persona" id="id_persona">
                  <input type="hidden" name="id_herramienta" value="{{$herramienta->id}}">
        ...
        <div class="mx-2">
            <div class="card">
                <div class="card-header bg-rojo text-white title">
                    <h3>Herramientas Ocupadas</h3>
                </div>
                <div class="card-body bg-camel"> 
                    @if($herramienta->asignadas)
                    <div class="table-responsive my-5 justify-content-center" id="resultado">
                        <table id="dataTable" class="table table-striped table-bordered" style="width:100%">
                            ...
                            <tbody>
                                @php($i=1)
                                @foreach($herramienta->asignadas as $he)
                                <tr>
                                    ...
                                    <td>
                                        <a href="#" onclick="editPrestar({{$he->id}},'{{$he->personal->nombre}}','{{$he->personal->apellidos}}','{{$he->cantidad}}','{{$he->fechaDevolucion}}','{{$he->observacion}}')" class="btn btn-outline-success" data-toggle="modal" data-target="#herramientaModal" title="Editar Herramienta" ><i class="fas fa-edit"></i></a>
                                        @if($he->fechaDevolucion==NULL)
                                        <a href="#" onclick="deletePrestar({{$he->id}})" class="btn btn-outline-danger" title="Eliminar Lugar" data-toggle="modal" data-target="#confirm"><i class="fas fa-trash-alt"></i></a>
                                        @endif
                                    </td>
                                </tr>
                                @php($i++)
                    ...
                    @else
                    <h4 class="text-danger">No se encuantra ninguna herramienta asignada...</h4>
                    @endif
                </div>                    
            </div>             
        </div>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body bg-camel">
            <form method="POST" action="{{route('herramienta.prestarEdit')}}" >
                @csrf
                <input type="hidden" name="id" id="presetarHerramienta_id" value="">
                <input type="hidden" name="id_herramienta" value="{{$herramienta->id}}">
                <div class="form-group">
...
<div class="modal fade" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" id="confirm-si">
  <div class="modal-dialog modal-md">
    <div class="modal-content">
      ...
      <form method="POST" action="{{route('herramienta.prestarDelete')}}">
          @csrf
          <input type="hidden" name="id" id="id_prestar" value="">
          <input type="hidden" name="herramienta" value="{{$herramienta->id}}">
          <div class="modal-footer">
